...
def tautologia(exp1):
    ...
    Essa função também é responsável por criar os conjuntos na CNF, quando o resultado do teste é False.
    ...
    for c in range(0, 2 ** n):
        ...
        if not r1[c][0]:
            r3aux = []
            for elemento in wff:
                if not valorBool[elemento]:
                    r3aux.append(elemento)
                else:
                    r3aux.append(oposto(elemento))
            r3.append(r3aux)
